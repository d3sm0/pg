...
def policy_iteration(env, pi_fn, pi_approx_fn, max_steps=10):
    ...
    while True:
        ...
        expected_return = (env.p0 * v).sum() / (1 - config.gamma)
        stats = {
        ...
        if global_step > max_steps:
            print("done")
            plot_policy_at_state(pi, action_label=["left", "right", "up", "down"], title="approx_pi", log_plot=True,
                                 step=global_step)
