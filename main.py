...
def policy_iteration(env, pi_fn, pi_approx_fn, max_steps=10, key_gen=None):
    ...
    for global_step in tqdm.trange(max_steps):
        ...
        pg_pi, pg_adv = get_pi(env, pi_old, pg, config.eta // 4)
        pi = pg_pi.clone()
        ...
        # if config.render:
        #    render(v, q, pi, pi_approx, landscape, global_step)
        # stats = {
...
def plot_loss_fn_surface(policies, env, title):
    ...
    for idx in range(1, n):
        ...
        v_pg = get_value(env, pg)
        v_ppo = get_value(env, ppo)
    ...
    config.tb.add_figure("policy_space", fig, global_step=config.max_steps)
...
def render(v, q, pi, pi_approx, landscape, global_step):
