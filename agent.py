...
class ActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, h_dim):
        ...
        self.v = nn.Sequential(nn.Linear(observation_space, 1))
        # nn.ELU(),
        # nn.Linear(h_dim, h_dim),
        # nn.ELU(),
        # nn.Linear(h_dim, 1))
        self.pi = nn.Sequential(nn.Linear(observation_space, h_dim),
                                nn.ELU(),
                                nn.Linear(h_dim, h_dim),
                                nn.ELU(),
...
class PG:
    def __init__(self, observation_space, action_space, h_dim):
        ...
        self.value_opt = optim.Adam(self._agent.v.parameters(), lr=config.vf_lr)
    ...
    def _train_pi(self, data_loader):
        for i in range(config.opt_epochs):
            ...
            for (s, a, r, s_prime, done_mask, probs_old) in data_loader:
                ...
                total_kl += kl / (len(data_loader))
                total_entropy += pi.entropy().mean() / (len(data_loader))
    ...
    def _train_value(self, data_loader):
        for ep in range(config.vf_opt_epochs):
            total_loss = 0
            for (s, a, r, s_prime, done_mask, _) in data_loader:
                delta = r + config.gamma * self._agent.value(s_prime) - self._agent.value(s)
                ...
                v_loss.backward()
                self.value_opt.step()
                total_loss += v_loss / len(data_loader)
            if total_loss < 0.02:
                break
        return {"train/v_loss": total_loss, "train/l2_reg": reg}
    ...
    def _train_pi(self, data_loader):
        for i in range(config.opt_epochs):
            ...
            for (s, a, r, s_prime, done_mask, probs_old) in data_loader:
                ...
                total_loss += loss.mean() / len(data_loader)
                total_kl += kl / len(data_loader)
                total_entropy += pi.entropy().mean() / len(data_loader)
                self.pi_opt.zero_grad()
                loss.mean().backward()
