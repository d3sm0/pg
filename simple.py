...
def main():
    ...
    v_best = np.load(f"plots/frozen_lake/v_frozen_lake.npy")

    def stop_criterion(t, v):
        return jnp.linalg.norm(v - v_best) < config.eps or t >= config.max_steps
    ...
    for agent, agent_fn in agents.items():
        ...
        for idx, eta in enumerate(etas):
            ax = axs[idx]
            e_pg, pg_pi, t, v_pg, pi_s, advs, _ = policy_iteration(agent_fn, env, eta, pi, stop_criterion)
