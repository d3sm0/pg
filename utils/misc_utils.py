...
def get_pi(env, pi_old, pi_fn, eta):
    ...
    return pi, adv, entropy, new_value, kl, d_s, _adv
...
def policy_iteration(agent_fn, env, eta, pi, stop_criterion):
    ...
    for step in itertools.count():
        pi, adv, e_pg, v, kl, d_s, avg_adv = get_pi(env, pi_old, agent_fn, eta=eta)
        data.append((pi, adv, v, d_s, avg_adv))
        if stop_criterion(step, v, v_last) or not is_prob_mass(pi):
            break
        step += 1
        pi_old = pi
        v_last = v.copy()
    pis, advs, vs, d_s, avg_adv = list(map(lambda x: jnp.stack(x, 0), list(zip(*data))))
    return e_pg, pi, step, v, pis, advs, vs, d_s #, avg_adv
